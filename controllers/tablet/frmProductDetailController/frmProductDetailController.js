define({ 
 ...
 //Type your controller code here Tablet
 ...
 populateReviewSegment:function(productReviews){
    ...
    for(var i=0;i<productReviews.length;i++){
    	var widthRating=this.calculateWidthForRating(productReviews[i].rating)+"%";
      	data[i]={
          lblName:{ text: productReviews[i].reviewer[0].name },
          lblRating:{ text:productReviews[i].rating },
          lblTitle: { text: productReviews[i].title },
          lblDate: { text: productReviews[i].submissionTime },
          lblDescription: { text: productReviews[i].comment },
          imgRating:{ src:"start.png"},
          flxUnfilledStart: { width:widthRating }
        };
    }
